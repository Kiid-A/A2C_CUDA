...
class A2C:
    ...
    def update(self, traj):
        ...
        if not observations.defined():
            raise RuntimeError("Observations tensor is undefined")
        if not actions.defined():
            raise RuntimeError("Actions tensor is undefined")
        if not rewards.defined():
            raise RuntimeError("Rewards tensor is undefined")
        if not returns.defined():
            raise RuntimeError("Returns tensor is undefined")
        ...
        for i in range(len(v_logp_e_p)):
            if not v_logp_e_p[i].defined():
                raise RuntimeError(f"Output tensor {i} from evaluate_actions is undefined")
        ...
        if not value_loss.defined() or not action_loss.defined() or not distEntropy_loss.defined():
            raise RuntimeError("One of the loss components is undefined")
