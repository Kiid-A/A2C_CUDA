...
# 绘制 SPE 图表
axes[0].bar(np.arange(len(labels)) - 0.2, spe_128, width=0.4, label=labels[0], color=colors[0], zorder=3)
axes[0].bar(np.arange(len(labels)), spe_1280, width=0.4, label=labels[1], color=colors[1], zorder=3)
axes[0].bar(np.arange(len(labels)) + 0.2, spe_12800, width=0.4, label=labels[2], color=colors[2], zorder=3)
axes[0].set_title('不同隐藏层大小下的 SPE 对比')
...
axes[1].set_title('不同隐藏层大小下的 MRT 对比')
...
axes[2].set_title('不同隐藏层大小下的 MUT 对比')
